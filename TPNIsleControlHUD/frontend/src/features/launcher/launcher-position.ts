...
export type LauncherRect = { left: number; right: number; top: number; bottom: number };
...
export function clampLauncherPosition(
  ...
  if (viewport.width < minimumStableViewport.width || viewport.height < minimumStableViewport.height ||
      baseRect.right <= baseRect.left || baseRect.bottom <= baseRect.top) return position;
  return {
