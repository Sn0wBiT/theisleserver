...
export function App() {
  ...
  return <main className="relative h-screen w-screen overflow-hidden">
    {runtimeError && <div className="absolute inset-0 grid place-items-center"><section className="hud-panel border border-rust p-5 text-sm text-rust">{runtimeError}</section></div>}
    {runtimeReady && <AuthGate><PositionStreamProvider><AuthenticatedHud /></PositionStreamProvider></AuthGate>}
    <NotificationLayer />
  </main>;
}
