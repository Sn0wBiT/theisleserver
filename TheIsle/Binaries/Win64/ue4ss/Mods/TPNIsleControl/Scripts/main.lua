...
local MOD_NAME = "TPNIsleControl"
local SAVED_DIR = "ue4ss/Mods/TPNIsleControl/Saved"
...
local function configureTransport()
    ...
    local ok, configured = pcall(function()
        return TPNIsleControlHttpConfigure(config.bridgeUrl, config.bridgeToken)
    end)
...
local function registerChatCommandHook()
    local ok, err = pcall(function()
        RegisterHook("/Script/TheIsle.TIPlayerController:ServerExecuteChatCommand",
            function(ctrlParam, commandParam)
                ...
                elseif command == "/human" or command == "human" then
    ...
    if ok then
        log("player chat command hook registered (/help, /quests, /human)")
    else
...
-- Human Generation 2 has moved between folders across EVRIMA cooks. Keep this
-- test command constrained to known playable human blueprints; never accept an
-- arbitrary class path from chat or the bridge because possessing a non-playable
-- pawn can crash the owning client.
local HUMAN_CLASS_PATHS = {
...
local function liveAddress(object)
...
local function findHumanClass()
    for _, path in ipairs(HUMAN_CLASS_PATHS) do
        local class
        pcall(function() class = StaticFindObject(path) end)
        if liveAddress(class) ~= nil then return class, path end
    end
    return nil, nil
end

local function destroyFreshActor(actor)
    if liveAddress(actor) == nil then return end
    pcall(function() actor:K2_DestroyActor() end)
end

local function actionHuman(steam, _args)
    if steam == nil or steam == "" then return false, "missing steam" end

    -- Resolve every gameplay object fresh inside the command-poll game-thread
    -- tick. Cached controller, pawn, world, or actor wrappers can become stale.
    local gm = findGameMode()
    ...
    local oldPawn = livePawnFromCtrl(ctrl)
    local oldAddress = liveAddress(oldPawn)
    if oldAddress == nil then return false, "player has no live dino" end
    if getSpecies(oldPawn):lower():find("human", 1, true) ~= nil then
        return true, "already human"
    end

    local humanClass, humanPath = findHumanClass()
    if humanClass == nil then
        return false, "playable human class unavailable (is bEnableHumans=true?)"
    end
    ...
    -- A short vertical retry ladder avoids terrain-collision spawn failures.
    local newPawn
    for _, offset in ipairs({ 300, 600, 1000 }) do
        pcall(function()
            newPawn = world:SpawnActor(
                humanClass,
                { X = x, Y = y, Z = z + offset },
                { Pitch = 0, Yaw = yaw, Roll = 0 }
            )
        end)
        if liveAddress(newPawn) ~= nil then break end
        newPawn = nil
    end
    if newPawn == nil then return false, "human spawn blocked at player location" end

    -- Establish the identity/registry bindings before possession so reconnect,
    -- persistence, and admin/RCON all agree that this is the player's character.
    local okPrepare, prepareErr = pcall(function()
        newPawn:SetReplicates(true)
        newPawn.bAlwaysRelevant = true
        newPawn:SetSteamId(steam)
        local characters = gm.AllPlayerCharacters
        characters[#characters + 1] = newPawn
    end)
    if not okPrepare then
        destroyFreshActor(newPawn)
        return false, "human possession failed; original dino untouched: " .. tostring(prepareErr)
    end

    local okPossess, possessErr = pcall(function() ctrl:Possess(newPawn) end)
    local possessed = livePawnFromCtrl(ctrl)
    if liveAddress(possessed) ~= liveAddress(newPawn) then
        destroyFreshActor(newPawn)
    ...
    -- The old pawn is known-fresh and exclusively ours now that possession was
    -- confirmed, so retire it in this same tick and remove its Steam binding.
    pcall(function() oldPawn:SetSteamId("") end)
    local okDestroy, destroyErr = pcall(function() oldPawn:K2_DestroyActor() end)

    -- These are post-swap repairs. Failure is reported as a warning because the
    -- player is already human and the original pawn must not be re-possessed.
    local warnings = {}
    if not okDestroy then warnings[#warnings + 1] = "old dino cleanup failed: " .. tostring(destroyErr) end

    local okHud, hudErr = pcall(function() ctrl:RequestOnRespawnHudUpdate() end)
    ...
    if #warnings > 0 then
        return true, "turned into human via " .. humanPath .. "; " .. table.concat(warnings, "; ")
    end
    return true, "turned into human via " .. humanPath
end
...
local function processCommand(line)
    ...
    if verb == "human" then
        pcall(function()
            privateChat(steam, "Human test: " .. tostring(msg or (success and "done" or "failed")))
        end)
    end
